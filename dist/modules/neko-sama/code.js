...
var source = (() => {
  ...
  // src/modules/neko-sama/neko-sama.ts
  var neko_sama_exports = {};
  __export(neko_sama_exports, {
    default: () => NekoSama
  });
      ...
      case "select": {
        ...
        if (!querying) {
          ...
          for (const val2 of values) {
            this.find('option[value="'.concat(val2, '"]')).attr("selected", "");
          }
          return this;
        }
  ...
  function parse(selector) {
    const subselects2 = [];
    const endIndex = parseSelector(subselects2, "".concat(selector), 0);
  ...
  var attributeRules = {
    ...
    element(next2, data2, options) {
      ...
      const regex = new RegExp("(?:^|\\s)".concat(escapeRegex(value), "(?:$|\\s)"), shouldIgnoreCase(data2, options) ? "i" : "");
  ...
  function html2(str) {
    ...
    return domEach(this, (el) => {
      ...
      const content = isCheerio(str) ? str.toArray() : this._parse("".concat(str), this.options, false, el).children;
      update(content, el);
    });
  }
  function toString() {
  ...
  function text2(str) {
    ...
    return domEach(this, (el) => {
      ...
      const textNode = new Text2("".concat(str));
      update(textNode, el);
    });
  }
  function clone() {
  ...
  function stringify(obj) {
    return Object.keys(obj).reduce((str, prop2) => "".concat(str).concat(str ? " " : "").concat(prop2, ": ").concat(obj[prop2], ";"), "");
  }
  ...
  function serialize() {
    ...
    const retArr = arr.map((data2) => "".concat(encodeURIComponent(data2.name), "=").concat(encodeURIComponent(data2.value)));
    ...
    for (const attr2 of treeAdapter.getAttrList(node)) {
      ...
      if (!attr2.namespace) {
        ...
        switch (attr2.namespace) {
          ...
          case NS.XLINK: {
            html3 += "xlink:".concat(attr2.name);
            break;
          }
          default: {
            html3 += "".concat(attr2.prefix, ":").concat(attr2.name);
          }
        }
      html3 += '="'.concat(escapeAttribute(attr2.value), '"');
    }
    return html3;
  }
  ...
  var BaseModule = class {
    ...
    getSettingValue(settingId) {
      this.settings.forEach((group) => {
        var _a2;
        return (_a2 = group.settings.find((setting) => setting.id === settingId)) == null ? void 0 : _a2.value;
      });
    }
  };

  // src/modules/neko-sama/scraper/homeScraper.ts
  var HomeScraper = class {
    constructor(baseName) {
      this.baseName = baseName;
    }
    async scrape() {
      const html3 = await request("".concat(this.baseName), "GET");
      this.$ = load(html3.body);
    ...
    animeDiscover() {
      const $2 = this.$;
      const animeDiscoverSec = $2("div.row.no-gutters:nth-last-child(-n+3)");
      const animeDiscoverItems = [];
      animeDiscoverSec.each(
        (_i, category) => {
          let titleCategory = $2(category).find("h1.nekosama.header").text().replaceAll("\n", "");
          let animeItems = $2(category).find("div.text-left");
          let data2 = [];
          animeItems.each((_i2, item) => {
            let title = $2(item).find("a .title").text();
            let url = $2(item).find("a").attr("href");
            let image = $2(item).find("img.lazy").attr("data-src");
            data2.push({
              url: url != null ? url : "",
              titles: { primary: title },
              poster: image != null ? image : ""
            });
          });
    ...
    scrapeRecentlyUpdated() {
      const $2 = this.$;
      const recentUpdatesSec = $2("#home > div > div.row.no-gutters.js-last-episode-container > div:nth-child(n)");
      const recentlyUpdatedItems = recentUpdatesSec.map((_i, anime) => {
        ...
        let image = animeRef.find("a > img").attr("src");
        if (image == void 0) {
          image = null;
        }
        ...
        return {
          ...
          titles: { primary: title },
          poster: (image == null ? void 0 : image.includes("https")) ? image : this.baseName + image,
          description: void 0,
          indicator: currentEpisode,
          current: currentEpisode.split("Ep. ")[1]
        };
      }).get();
      return {
        type: 3 /* GRID_3x */,
        title: "Derniers \xE9pisodes",
  ...
  // src/modules/neko-sama/neko-sama.ts
  var NekoSama = class extends BaseModule {
    constructor() {
      super(...arguments);
      this.baseUrl = "https://neko-sama.fr";
      this.metadata = {
        id: "nekosama",
        name: "Neko-sama",
    ...
    async search(query, page) {
      ...
      const resp = await request("".concat(VOSTFR), "GET");
      const resp2 = await request("".concat(VF), "GET");
      const data2 = JSON.parse(resp.body);
      const data22 = JSON.parse(resp2.body);
      let items = [];
      let totalPages = 1;
      if (query) {
        items = data2.filter((item) => item.title.toLowerCase().includes(query.toLowerCase()) || item.others.toLowerCase().includes(query.toLowerCase()));
        items = items.concat(data22.filter((item) => item.title.toLowerCase().includes(query.toLowerCase()) || item.others.toLowerCase().includes(query.toLowerCase())));
      }
      items = items.map((item) => {
        return {
          url: item.url,
          titles: {
            primary: "".concat(item.title, " ").concat(item.url.includes("vostfr") ? "VOSTFR" : "VF"),
    ...
    async info(_url) {
      var _a2, _b, _c;
      const resp = await request("".concat(_url), "GET");
      const $2 = load(resp.body);
      const title = $2("h1").text();
      const altTitles = $2("small").text().split(", ");
      const description = $2(".synopsis > p").text();
      const poster = $2(".cover > img").attr("src");
      const banner = (_a2 = $2("#head").css("background-image")) == null ? void 0 : _a2.replace(/url\((['"])?(.*?)\1\)/gi, "$2");
      const score = parseInt($2(".item:contains('Score moyen')").text().split("\n")[3].split("/")[0]);
      const status = $2(".item:contains('Status')").text().split(" ")[1];
      const startDate = (_c = (_b = $2(".item:contains('Diffusion')").text().match(/\d+/)) == null ? void 0 : _b[0]) != null ? _c : "0";
      return {
        titles: {
          primary: title,
        ...
        banner,
        status: status == "Pas" ? 3 /* NOT_RELEASED */ : 4 /* UNKNOWN */,
        rating: score,
    ...
    async media(_url) {
      const resp = await request("".concat(_url), "GET");
      const $2 = load(resp.body);
      let episodes = $2(".episodes .col-lg-12");
      let title = $2("h1").text();
      let episodesList = [];
      episodes.each((_, episode) => {
        var _a2, _b;
        const episodeRef = $2(episode);
        const episodeTitle = episodeRef.find("a").text();
        const episodeUrl = (_a2 = episodeRef.find("a").attr("href")) != null ? _a2 : "";
        const episodeNumber = parseInt((_b = episodeTitle.split(" - ").pop()) != null ? _b : "0");
        episodesList.push({
          title: episodeTitle,
          url: episodeUrl,
    ...
    async sources(_url) {
      ...
      const $2 = load(resp.body);
      let servers = [];
      let link = $2("body").text().match(/https:\/\/fusevideo.io\/e\/[a-zA-Z0-9]+/g);
      if (link) {
        servers.push({
          name: "FuseVideo",
          url: link[0]
    ...
    async streams(_url) {
      ...
      const $2 = load(resp.body);
      const script = $2("head").find("script").map((_, elem) => $2(elem).attr("src")).get().find((src) => src == null ? void 0 : src.includes("https://fusevideo.io/f"));
      if (!script) {
        throw new Error("No script found");
      }
      const linkScript = await request(script, "GET");
      let videoLinkJSON = (_a2 = linkScript.body.match(/atob\("([^"]+)"\)/gm)) == null ? void 0 : _a2[0];
      videoLinkJSON = videoLinkJSON == null ? void 0 : videoLinkJSON.replace('atob("', "").replace('")', "");
      videoLinkJSON = atob(videoLinkJSON);
      videoLinkJSON = videoLinkJSON == null ? void 0 : videoLinkJSON.replace(/\\|\\|\\|/, "").slice(32);
  ...
  return __toCommonJS(neko_sama_exports);
})();
